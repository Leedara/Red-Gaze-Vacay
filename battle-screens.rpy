...
screen battle_menu():
    ...
    hbox:
        ...
        if showbattlemenu:
            textbutton _("{color=#0080c0}Attack{/color}"):
                action Jump("battle_attack")
                tooltip _("{color=#0080c0}Do like the honey badgers and just go for the balls!")

            textbutton _("{color=#0080c0}Nernas Flareshot{/color}"):
                action Jump("battle_attackk")
                tooltip _("{color=#0080c0}A lesser version of Nernas Sunshot!{/color}")

            textbutton _("{color=#0080c0}Steely Resolve{/color}"):
                action Jump("battle_defend")
                tooltip _("{color=#0080c0}Focus your mind and muscles! Halves incoming damage.{/color}")
            if check_inv_for(battle_items):
                textbutton _("{color=#0080c0}Items{/color}"):
                    action SetScreenVariable("showbattlemenu", False)
                    tooltip _("{color=#0080c0}Use some of your tools!{/color}")
            ...
            if "item_sugar" in inv:
                textbutton _("{color=#0080c0}{size=-8}Steely Sugar (throw){/size}{/color}"):
                    ...
                    tooltip _("{color=#0080c0}Do you really wanna try that?{/color}")
                # add new items here
            ...
            if "item_heart" in inv:
                textbutton _("{color=#0080c0}{size=-8}Yulian Heart (eat){/size}{/color}"):
                    action Jump("battle_heart")
                    tooltip _("{color=#0080c0}It's a herb that cures Slow...{/color}")

            textbutton _("{color=#0080c0}Cancel{/color}"):
                action SetScreenVariable("showbattlemenu", True)
                tooltip _("{color=#0080c0}Close the item menu.{/color}")
